defmodule Lenra.LegalTest do
  ...
  describe "accept" do
    ...
    test "existing cgu", %{user: user} do
      {:ok, cgu} = Repo.insert(CGU.new(%{link: "test.html", hash: "a", version: 2}))
    ...
    test "not latest cgu", %{user: user} do
      ...
      Repo.insert(
        %{link: "test2.html", hash: "b", version: 3}
        |> CGU.new()
    ...
    test "latest cgu", %{user: user} do
      Repo.insert(CGU.new(%{link: "test.html", hash: "a", version: 2}))
  ...
  describe "user_accepted_latest_cgu?" do
    ...
    test "User did not accept CGU" do
      {:ok, %{inserted_user: user}} = UserTestHelper.register_john_doe()
      %{link: "a", version: 2, hash: "a"} |> CGU.new() |> Repo.insert()
    ...
    test "User accepted latest CGU" do
      {:ok, %{inserted_user: user}} = UserTestHelper.register_john_doe()
      {:ok, cgu} = %{link: "a", version: 2, hash: "a"} |> CGU.new() |> Repo.insert()
