defmodule UserServicesTest do
  ...
  test "validate dev with invalid uuid type" do
    ...
    assert {:error, %Lenra.Errors.BusinessError{reason: :invalid_uuid}} =
  ...
  test "validate dev with invalid code" do
    ...
    assert {:error, %Lenra.Errors.BusinessError{reason: :invalid_code}} =
             Accounts.validate_dev(user, invalid_code)
  end
  ...
  test "validate dev a user that is already a dev" do
    ...
    assert {:error, %Lenra.Errors.BusinessError{reason: :already_dev}} =
             Accounts.validate_dev(user, valid_code)
  end
